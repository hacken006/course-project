export default {
    courses: [
        {
        name: "HTML, CSS từ zero đến Hero",
        slug: "html-css",
        image: "html-css.jpg",
        jointed: 8898,
        menus: "HTML/ CSS",
        id: 1,
        description:
          "Khóa học đề cao việc thực hành qua những ví dụ trong thực tế giúp học viên nhanh chóng xây dựng được giao diện website",
        learning: [
            {
                name: "HTML 1",
                slug: "lession1",
                link: "https://www.youtube-nocookie.com/embed/R6plN3FvzFY"
            },
            {
                name: "HTML 2",
                slug: "lession2",
                link: "https://www.youtube-nocookie.com/embed/f5hbmw7Ba7c"
            },
            {
                name: "HTML 3",
                slug: "lession3",
                link: "https://www.youtube-nocookie.com/embed/zwsPND378OQ"
            },
            {
                name: "HTML 4",
                slug: "lession4",
                link: "https://www.youtube-nocookie.com/embed/7BJiPyN4zZ0"
            },
            {
                name: "HTML 5",
                slug: "lession5",
                link: "https://www.youtube-nocookie.com/embed/ZotVkQDC6mU"
            },
            {
                name: "HTML 6",
                slug: "lession6",
                link: "https://www.youtube-nocookie.com/embed/LYnrFSGLCl8"
            },
        ]
        },
        {
        name: "Xây dựng web responsive",
        slug: "responsive-web-design",
        image: "responsive-web-design.jpg",
        jointed: 3316,
        menus: "Responsive",
        id: 2,
        description:
            "Khóa học này sẽ giúp bạn nắm chắc tư tưởng cốt lõi của việc xây dựng giao diện website responsive và áp dụng trong thực tế",
        learning: [
            {
                name: "CSS 1",
                slug: "lession1",
                link: "https://www.youtube-nocookie.com/embed/uz5LIP85J5Y"
            },
            {
                name: "CSS 2",
                slug: "lession2",
                link: "https://www.youtube-nocookie.com/embed/f5hbmw7Ba7c"
            },
            {
                name: "CSS 3",
                slug: "lession3",
                link: "https://www.youtube-nocookie.com/embed/5QT0aeovTTY"
            },
            {
                name: "CSS 4",
                slug: "lession4",
                link: "https://www.youtube-nocookie.com/embed/CIIYogDrGto"
            },
            {
                name: "CSS 5",
                slug: "lession5",
                link: "https://www.youtube-nocookie.com/embed/XJiq_d0vGCQ"
            },
            {
                name: "CSS 6",
                slug: "lession6",
                link: "https://www.youtube-nocookie.com/embed/YgkzJkmDP3U"
            },
        ]
        },
        {
        name: "Lập trình Javascript cơ bản (bản đầy đủ)",
        slug: "javascript-co-ban",
        image: "javascript-co-ban.jpg",
        jointed: 7355,
        menus: "Javascript",
        id: 3,
        description:
            "Khóa học Javascript cơ bản dành cho mọi đối tượng bắt đầu học lập trình với nội dung ngắn gọn, dễ hiểu và hệ thống làm bài tập phong phú",
        learning: [
            {
                name: "Javascript 1",
                slug: "lession1",
                link: "https://www.youtube-nocookie.com/embed/0SJE9dYdpps"
            },
            {
                name: "Javascript 2",
                slug: "lession2",
                link: "https://www.youtube-nocookie.com/embed/f5hbmw7Ba7c"
            },
            {
                name: "Javascript 3",
                slug: "lession3",
                link: "https://www.youtube-nocookie.com/embed/jV06pqjUUc"
            },
            {
                name: "Javascript 4",
                slug: "lession4",
                link: "https://www.youtube-nocookie.com/embed/jV06pqjUUc"
            },
            {
                name: "Javascript 5",
                slug: "lession5",
                link: "https://www.youtube-nocookie.com/embed/efI98nT8Ffo"
            },
            {
                name: "Javascript 6",
                slug: "lession6",
                link: "https://www.youtube-nocookie.com/embed/W0vEUmyvthQ"
            },
        ]
        },
        {
        name: "Xây dựng web với NodeJS & ExpressJS",
        slug: "nodejs",
        image: "nodejs.jpg",
        jointed: 2038,
        menus: "NodeJS",
        id: 4,
        description:
            "Xây dựng web trong thực tế với NodeJS & ExpressJS với cách chia sẻ chi tiết, tận tâm, dễ hiểu và chất giọng giàu sức sống của người chia sẻ",
        learning: [
            {
                name: "NodeJS 1",
                slug: "lession1",
                link: "https://www.youtube-nocookie.com/embed/zwsPND378OQ"
            },
            {
                name: "NodeJS 2",
                slug: "lession2",
                link: "https://www.youtube-nocookie.com/embed/zwsPND378OQ"
            },
            {
                name: "NodeJS 3",
                slug: "lession3",
                link: "https://www.youtube-nocookie.com/embed/zwsPND378OQ"
            },
            {
                name: "NodeJS 4",
                slug: "lession4",
                link: "https://www.youtube-nocookie.com/embed/zwsPND378OQ"
            },
            {
                name: "NodeJS 5",
                slug: "lession5",
                link: "https://www.youtube-nocookie.com/embed/zwsPND378OQ"
            },
            {
                name: "NodeJS 6",
                slug: "lession6",
                link: "https://www.youtube-nocookie.com/embed/zwsPND378OQ"
            },
        ]
        },
        {
        name: "HTML, CSS tips",
        slug: "html-css-tutorials",
        image: "html-css-tutorials.jpg",
        jointed: 1536,
        menus: "HTML Tips",
        id: 5,
        description:
            "Tutorials về HTML, CSS, UI, UX sẽ được tổng hợp tại khóa học này, các video có nội dung ngắn gọn, súc tích giúp học viên có thể ứng dụng ngay vào thực tế",
        learning: [
            {
                name: "Tips 1",
                slug: "lession1",
                link: "https://www.youtube-nocookie.com/embed/zwsPND378OQ"
            },
            {
                name: "Tips 2",
                slug: "lession2",
                link: "https://www.youtube-nocookie.com/embed/zwsPND378OQ"
            },
            {
                name: "Tips 3",
                slug: "lession3",
                link: "https://www.youtube-nocookie.com/embed/zwsPND378OQ"
            },
            {
                name: "Tips 4",
                slug: "lession4",
                link: "https://www.youtube-nocookie.com/embed/zwsPND378OQ"
            },
            {
                name: "Tips 5",
                slug: "lession5",
                link: "https://www.youtube-nocookie.com/embed/zwsPND378OQ"
            },
            {
                name: "Tips 6",
                slug: "lession6",
                link: "https://www.youtube-nocookie.com/embed/zwsPND378OQ"
            },
        ]            
        }
    ],
  };
  
